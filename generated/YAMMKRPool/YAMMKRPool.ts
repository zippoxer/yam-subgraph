...
export class YAMMKRPool extends ethereum.SmartContract {
  static bind(address: Address): YAMMKRPool {
    return new YAMMKRPool("YAMMKRPool", address);
  }
  ...
  mkr(): Address {
    let result = super.call("mkr", "mkr():(address)", []);

    return result[0].toAddress();
  }

  try_mkr(): ethereum.CallResult<Address> {
    let result = super.tryCall("mkr", "mkr():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
